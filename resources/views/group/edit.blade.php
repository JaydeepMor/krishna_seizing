...
@section('content')
    ...
    <div class="content-header">
        <div class="header-section">
            <h1>
                <i class="gi gi-table"></i>{{ __('Edit Group') }}
            </h1>
        </div>
    </div>
    <!-- END Page Header -->
                                    ...
                                    <div class="form-group">
                                        <select name="finance_company_id[]" id="finance_company_id" multiple="multiple" class="form-control multiselect-inline">
                                            @if (!empty($financeCompanies))
                                                @php
                                                    $groupFinances = (!empty($row->groupFinances) && !$row->groupFinances->isEmpty()) ? $row->groupFinances->pluck('finance_company_id')->toArray() : [];
                                                @endphp

                                                @foreach ($financeCompanies as $financeCompany)
                                                    @php
                                                        $selected = (in_array($financeCompany->id, old('finance_company_id', $groupFinances))) ? 'selected' : '';
                                                    @endphp
